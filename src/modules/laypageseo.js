...
layui.define(function(exports){
  ...
  //字符常量
  ,MOD_NAME = 'laypageseo', DISABLED = 'layui-disabled'
  
  //构造器
  ,Class = function(options){
    var that = this;
    that.config = options || {};
    that.config.index = ++laypageseo.index;
    that.render(true);
  };
        ...
        for(; start <= end; start++){
          if(start === config.curr){
            ...
            if(config.template){
              ...
              var link = that.template(config.template, params)
              pager.push('<a is-template="true" href="'+link+'" data-page="'+ start +'">'+ start +'</a>');
            }else{
  ...
  Class.prototype.jump = function(elem, isskip){
    ...
    for(var i = 0, len = childs.length; i < len; i++){
      var ele = childs[i]
      if(ele.nodeName.toLowerCase() === 'a' && !ele.getAttribute('is-template')){
        laypageseo.on(ele, 'click', function(){
    ...
    //条数
    if(select){
      laypageseo.on(select, 'change', function(){
    ...
    //确定
    if(btn){
      laypageseo.on(btn, 'click', function(){
        skip();
      });
    }
  };
  ...
  Class.prototype.skip = function(elem){
    ...
    if(!input) return;
    laypageseo.on(input, 'keyup', function(e){
      var value = this.value
      ,keyCode = e.keyCode;
  ...
  // 模板
  Class.prototype.template = function(template, params) {
  ...
  var laypageseo = {
    ...
    ,index: layui.laypageseo ? (layui.laypageseo.index + 10000) : 0
